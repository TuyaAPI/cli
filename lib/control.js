...
function parseConfig(config, options) {
	...
	if (options.key === undefined && config.get(options.id) !== undefined) {
		if (options.ip === undefined) {
			return new TuyAPI({version: 3.3, id: options.id, key: config.get(options.id)});
		}

		return new TuyAPI({version: 3.3, ip: options.ip, id: options.id, key: config.get(options.id)});
	}
	...
	if (options.key === undefined && config.get(options.id) === undefined) {
		...
		if (options.ip === undefined) {
			return new TuyAPI({version: 3.3, id: options.id, key: options.key});
		}

		return new TuyAPI({version: 3.3, ip: options.ip, id: options.id, key: options.key});
	}
}
